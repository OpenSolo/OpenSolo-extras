...
// Initialize the PID_GRANDO_CONTROLLER module for Iq 
	pid1_iq.param.Kp = _IQ(3.176*BASE_CURRENT/BASE_VOLTAGE);
...
// Initialize the PID_GRANDO_CONTROLLER module for Speed 
    pid1_spd.param.Kp = _IQ(0.2*BASE_FREQ/BASE_CURRENT/(POLES/2));
...
// ------------------------------------------------------------------------------	
    PwmDacCh1 = _IQtoQ15(clarke1.As);
    PwmDacCh2 = _IQtoQ15(smo1.Theta); 
    PwmDacCh3 = _IQtoQ15(rg1.Out); 
