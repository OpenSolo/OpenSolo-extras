...
enum
{
  ...
  PROP_QUALITY,
  PROP_FILTER_LENGTH,
  PROP_SN_RC_INTF_SHM_ID //sn_specifc
};
...
static shmem_audio_struct* shmem_audio = 0;	
...
gst_audio_resample_class_init (GstAudioResampleClass * klass)
{
  ...
  //sn_specific ////////////////////////////////////////////////
  g_object_class_install_property (gobject_class, PROP_SN_RC_INTF_SHM_ID,
    g_param_spec_int ("shmid", "ShmId-Ctrl-Rc",
          "ShmId for SN RC_CTRL (v3)", 0, G_MAXINT,0,
    G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
gst_audio_resample_check_discont (GstAudioResample * resample, GstBuffer * buf)
{
  ...
  //pcnt++; if(pcnt>100) pcnt=0;
  /* is the incoming buffer a discontinuity? */
  if (G_UNLIKELY (GST_BUFFER_IS_DISCONT (buf))){
/*
if(pcnt>90) GST_WARNING_OBJECT (resample,"dump0: %d %d %d %d\n",
...
//if(pcnt>90) GST_WARNING_OBJECT (resample,"dump1: %d %d %d %d\n",
//GST_BUFFER_TIMESTAMP (buf),
//resample->t0, resample->inrate, GST_SECOND);
    return FALSE;
   }
...
//if(pcnt>90) GST_WARNING_OBJECT (resample,"dump2: %d %d %d %d %d\n",
//GST_BUFFER_TIMESTAMP (buf),
//resample->t0, resample->inrate, GST_SECOND,delta);
    return FALSE;
}
  if (shmem_audio && -1!=(int)shmem_audio) 
    shmem_audio->resample_error=shmem_audio->resample_error + 1;
...
/*
if(pcnt>90) GST_WARNING_OBJECT (resample,"dump3: %d %d %d %d\n",
GST_BUFFER_TIMESTAMP (buf),
resample->t0, resample->inrate, GST_SECOND);
*/
  return TRUE;
...
pcnt++; if(pcnt>100) pcnt=0;
  if(pcnt>90)
  //GST_LOG_OBJECT  
GST_WARNING_OBJECT (resample,  "transforming buffer of %ld bytes, ts %"
      GST_TIME_FORMAT ", duration %" GST_TIME_FORMAT ", offset %"
      G_GINT64_FORMAT ", offset_end %" G_GINT64_FORMAT "end\n",
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SN_RC_INTF_SHM_ID:
      resample->shmid = g_value_get_int (value);
      GST_DEBUG ("AudioResample: Shm ID =%u", resample->shmid);
      if (shmem_audio && -1!=(int)shmem_audio) {
			GST_WARNING("Audioresample: Shm isn't null...");
	      shmdt((void*)shmem_audio); }
  		shmem_audio = (shmem_audio_struct*)shmat(resample->shmid,NULL,0);
  		if((unsigned int)shmem_audio == -1) {
    		GST_ERROR ("audioresample: Shared memory attach failed...");
 		  }
      GST_WARNING ("audioresample: Shared memory attached");
							#if _USE_SEMAPHORE_
							MFW_WEAK_ASSERT(0<shmem_audio->semid_snd);
							GST_DEBUG (">>VPU_ENC: Sem ID=%u", shmem_audio->semid_snd);
								semid_snd= shmem_audio->semid_snd;
							#endif
    break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_FILTER_LENGTH:
      ...
      break;
    case PROP_SN_RC_INTF_SHM_ID:
      g_value_set_int (value, resample->shmid);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
