...
/////////////////////////////////// bookmark ////////////////////////////////////////
  SN_RC_INTF_SHM_ID,	// property to assign shmid from client/manager master thread, liyenho
};
...
static void luma_subsample(guint8 *capbuf,
			guint8 *subsampbuf,  /*no cropping support yet...*/
			gint capture_width,
			gint capture_height);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case SN_RC_INTF_SHM_ID:	
    {
      ...
      GST_DEBUG (">>TVSrc: Shm ID =%u", shmid);
      if (stats_sn && -1!=(int)stats_sn) {
			GST_WARNING("TVSrc: Enc Shm isn't null...");
	      shmdt((void*)stats_sn); }
  		stats_sn = (intf_enc_rc_sn*)shmat(shmid,NULL,0);
  		if((unsigned int)stats_sn == -1) {
    		GST_ERROR ("TVSrc: Shared memory attach failed...");
    		MFW_WEAK_ASSERT(0);	// don't apply exit() @ run time...
 		}
		GST_WARNING("TVSrc: Enc Shm attached");
#if _USE_SEMAPHORE_
    	MFW_WEAK_ASSERT(0<stats_sn->semid_snd);
      GST_DEBUG (">>TVSrc: Sem ID =%u", stats_sn->semid_snd);
	  		semid_snd= stats_sn->semid_snd;
#endif
    if (stats_sn && -1!=(int)stats_sn)
      stats_sn->version =  (stats_sn->version & 0xfffffff0) | SN_VERSION;
    	break;
	 }
  }
...
/////////////////////////////////// bookmark ////////////////////////////////////////
	if (stats_sn && -1!=(int)stats_sn) {
		shmdt ((void*)stats_sn); 
		GST_ERROR("TVSrc: Enc Shm detached");
	}
  return TRUE;
}

/////////////////////////////////// bookmark ////////////////////////////////////////
/*1280x720 p image : (1280/32, 720/16) -\E0 40 x 45
  720x576 p image: (720/16, 576/16)    -> 45x36
  720x480 p image: (720/16, 480/16)    -> 45x30*/
static void luma_subsample(guint8 *capbuf,
			guint8 *subsampbuf,  /*no cropping support yet...*/
			gint capture_width,
			gint capture_height)
{
  gint ofr, ofc, i, r, c;
	MFW_WEAK_ASSERT(
	   (720==capture_height || 576==capture_height || 480==capture_height) &&
	   (1280==capture_width || 720==capture_width));
	switch (capture_height) {
		case 720: ofc = 32; ofr = 16*1280;
			for (i=r=0; r<1280/32; r+=1) {
				for (c=0; c<720/16; c+=1) {
					*subsampbuf++ = capbuf[i];
					i+= ofc;
				}
				i= r*ofr;
			...
			break;
		case 576:
		case 480: ofc = 16; ofr = 16*720;
			for (i=r=0; r<720/16; r+=1) {
				for (c=0; c<capture_height/16; c+=1) {
					*subsampbuf++ = capbuf[i];
					i+= ofc;
				}
				i= r*ofr;
			...
			break;
		default:
			GST_ERROR( ">>TV_SRC: luma_subsample() failed...");
			break;
	}
}
...
#endif
		stats_sn->enc.frames_ts_in[stats_sn->enc.mini_frm_idx] = ts;

		switch(v4l_src->capture_height) {
		   case 720: //1280*720
		      stats_sn->minifrmsz = 40*45;
		      break;
		   case 576: // 720*576
		      stats_sn->minifrmsz = 45*36;
		      break;
		   case 480: // 720*480
		      stats_sn->minifrmsz = 45*30;
		      break;
		}
...
/////////////////////////////////// bookmark ////////////////////////////////////////
	  //GST_ERROR("TVSrc: Installing 'shmid' property");	// for debug... liyenho
