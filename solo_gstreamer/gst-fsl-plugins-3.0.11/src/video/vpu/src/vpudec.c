...
#define   SN_VERSION 0x200
#define   SNRTPTS_DECODE
/* FIX ME */
...
void varstore(const char* filename, int *valbuff1, int *valbuff2, int *valbuff3, int *cnt,
              int cntstart, int size, int val1, int val2, int val3);
...
static GstsutilsOptionEntry g_vpudec_option_table[] = {
  ...
  {SN_RC_INTF_SHM_ID, "shmid", "ShmId-Ctrl-Rc",
          "ShmId for SN RC_CTRL", G_TYPE_INT,
          	G_STRUCT_OFFSET (VpuDecOption, shmid), "0", "0", STR_MAX_INT},
  /* terminator */
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  if (SN_RC_INTF_SHM_ID==prop_id) {	
      GST_DEBUG (">>VPU_DEC: Shm ID =%u", self->options.shmid);
      if (stats_sn && -1!=(int)stats_sn) {
			GST_WARNING("VPU_DEC: Shm isn't null...");
	      shmdt((void*)stats_sn); }
  		stats_sn = (intf_dec_rc_sn*)shmat(self->options.shmid,NULL,0);
  		if((unsigned int)stats_sn == -1) {
    		GST_ERROR ("VPU_DEC: Shared memory attach failed...");
    		MFW_WEAK_ASSERT(0);	// don't apply exit() @ run time...
 		}
    if (stats_sn && -1!=(int)stats_sn)  
      stats_sn->version = (stats_sn->version & 0xfffff0ff) | SN_VERSION;
      
  GST_ERROR("VPU_DEC: Shared memory attached");
#if _USE_SEMAPHORE_
    	MFW_WEAK_ASSERT(0<stats_sn->semid_rec);
      GST_DEBUG (">>VPU_DEC: Sem ID=%u", stats_sn->semid_rec);
	  		semid_rec= stats_sn->semid_rec;
#endif
	 }
  return;
}
...
gst_vpudec_core_check_display_queue (GstVpuDec * vpudec)
{
  ...
  /**/
  struct timespec tm;
  static int tprev,tcurr,tdelta;
  clockid_t ttype = CLOCK_MONOTONIC;
  ...
  for (i = 0; i < vpudec->frame_num; i++) {
    if ((vpudec->frames[i].display_handle)      //){
      ...
      if (gst_buffer_is_writable (vpudec->frames[i].gstbuf)) {
        ...
        vpudec->output_size--; 
        //GST_WARNING("DBG:output_size Decrement=%d", vpudec->output_size);
        hiti=i;
        cleared_cnt++;
												#ifdef MX6_CLEARDISPLAY_WORKAROUND
																return cleared_cnt; //clear only once
												#endif
      }//end writable
    } //end display_handle
  }//end for
...
gst_vpudec_finalize (GObject * object)
{
  ...
  G_OBJECT_CLASS (parent_class)->finalize (object);
	...
	if (stats_sn && -1!=(int)stats_sn) {
		shmdt ((void*)stats_sn); 
  	GST_ERROR("VPU_DEC: Shared memory detached"); 
	}
}
    ...
    VpuFrameBuffer * p)
{
  ...
  if ((vpudec->mosaic_cnt == 0)
    ...
    if (dropcntmask = (vpudec->drop_level & SKIP_NUM_MASK)) {
      if (//(vpudec->vpu_stat.out_cnt & dropcntmask)
           (vpudec->vpu_stat.out_cnt & 0x1ff) //0xff> 256*8/60 = 34sec for 60fps case
         ) {
        display = TRUE;
      } else
      {
  ...
  if (display) {
    ...
    blockstate = gst_pad_is_blocking(vpudec->srcpad);  
    if(blockstate == FALSE)  
...
gst_vpudec_process_error(GstVpuDec * vpudec, VpuDecRetCode vpuretcode)
{
  ...
  switch (err){
    ...
    case VPU_DEC_ERR_CORRUPT:
      ...
      break;
    default:
      //tri GST_ELEMENT_ERROR(vpudec, STREAM, FAILED, ("unknown error detect"), (NULL));
      break;
  };
...
gst_vpudec_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  gboolean retry;
  struct timespec tm;
  static int tprev,tcurr, t00,t0,t1, t2, t3,t4,t5;
  clockid_t ttype = CLOCK_MONOTONIC; 
  #ifdef SNRTPTS_DECODE
  const char snrtpts_decode = 1;
  #else
  const char snrtpts_decode = 0;
  #endif
  char dbgbreakflag = 0;
  int nsize_init= 0;

  clock_gettime(ttype, &tm); 
  t00=(int)(tm.tv_nsec)/1000;
  ...
  if (buffer) {
    vpudec->vpu_stat.in_cnt++;
      ...
      if (stats_sn) stats_sn->decoding = stats_sn->decoding +1;	
  ...
  clock_gettime(ttype, &tm);
  t1=(int)(tm.tv_nsec)/1000;
  nsize_init= inbuf.nSize;
  /*
  {
     static int valbuff1[2020], valbuff2[2020], valbuff3[2020],vscnt=0; 
     struct timespec tm1;
     int tcurr1,tdiff,headflag;
     static int tcurrp;
     clockid_t ttype1 = CLOCK_MONOTONIC;
     clock_gettime(ttype1, &tm1);
     tcurr1 = (int)(tm1.tv_nsec)/1000;
     tdiff=(tcurr1>tcurrp)?(tcurr1-tcurrp):(tcurr1+1000000-tcurrp);
     if(inbuf.nSize>15000) headflag=90000000; else headflag=0;
       varstore("/var/volatile/tmp/cap2", valbuff1, valbuff2,valbuff3, &vscnt, 0, 2000,inbuf.nSize+headflag,
         tcurr1,tdiff);
       if(inbuf.nSize>0)tcurrp = tcurr1;
  }
  */
    ...
    CORE_API (VPU_DecDecodeBuf, //<<<<<<<<<<<<<<<<<<<<<<<<<DECODE<<<<<<<<<<<<<<<<<<<<<<<<<<<
          {
          if (core_ret == VPU_DEC_RET_FAILURE_TIMEOUT)
        CORE_API (VPU_DecReset,, core_ret, vpudec->context.handle);
  if (stats_sn) stats_sn->decoding = 0;	
        goto bail;}
        , core_ret, vpudec->context.handle, &inbuf 
        , &core_buf_ret);

  clock_gettime(ttype, &tm); 
  t2=(int)(tm.tv_nsec)/1000;

  if ((core_buf_ret & VPU_DEC_OUTPUT_MOSAIC_DIS)&&(snrtpts_decode==1)) 
    core_buf_ret = (core_buf_ret & (~VPU_DEC_OUTPUT_MOSAIC_DIS))| VPU_DEC_OUTPUT_DIS;
  ...
  /*
  {
     static int valbuff1[320], valbuff2[320], valbuff3[320],vscnt=0; 
     struct timespec tm1;
     int tcurr1;
     clockid_t ttype1 = CLOCK_MONOTONIC;
     clock_gettime(ttype1, &tm1);
     tcurr1 = (int)(tm1.tv_nsec)/1000;
       varstore("/var/volatile/tmp/cap5", valbuff1, valbuff2,valbuff3, &vscnt, 0, 300, 
         tcurr1, oframe, retrycnt);
  }
  */
      if (oframe) {
        retrycnt = 0;
      ...
      if (oframe) {
        ...
        if (oframe->display_handle == NULL) {
          clock_gettime(ttype, &tm);
          t5=(int)(tm.tv_nsec)/1000;
          VPU_ASSIGN_OUTPUT (vpudec, oframe, oinfo.pDisplayFrameBuf);
          //not frequently called case here
          ret = gst_vpudec_show_frame (vpudec, oframe, oinfo.pDisplayFrameBuf);
        } else {
          GST_WARNING ("Frame %d still in displaying queue!!", oframe->id);
          TSManagerSend (vpudec->tsm);
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_NEWSEGMENT:
    {
      ...
      if (format == GST_FORMAT_TIME) {
        ...
        //GST_WARNING("m=%d ai=%d fifo=%d ",vpudec->tsm_mode,MODE_AI,MODE_FIFO);
...
void varstore(const char* filename, int *valbuff1, int *valbuff2, int *valbuff3, 
              int *cnt, 
              int cntstart, int size, int val1, int val2, int val3)
{
  int i;
  FILE * fd;
  int cntlocal = *cnt;
  if(cntlocal==0) for(i=0;i<size;i++) {valbuff1[i]=0; valbuff2[i]=0;}
  
  if(cntlocal < (cntstart+size+5)) cntlocal= cntlocal + 1;
  if((cntlocal >= cntstart)&&(cntlocal<(cntstart+size))) {
    valbuff1[cntlocal-cntstart]=val1;
    valbuff2[cntlocal-cntstart]=val2;
    valbuff3[cntlocal-cntstart]=val3;
    }
  
  if(cntlocal == (cntstart+size)) {
    //save data
    fd=fopen(filename, "w");
    for(i=0;i<size;i++)
      fprintf(fd,"%d: %d %d %d\n",i,valbuff1[i], valbuff2[i], valbuff3[i]);
    fclose(fd);
  }
  *cnt = cntlocal;
}
