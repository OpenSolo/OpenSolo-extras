 ...
 * The messages structure contains one field:
 * <itemizedlist>
 * <listitem>
 *   <para>
 *   #guint64
...
#define SN_VERSION 0x300000
#define STATS_SN_INVALID  12344321
...
#define UDP_DEFAULT_SHMID              1212
#define FORCEI_ERR_REC_CNT             5
...
/////////////////////////////////// bookmark ////////////////////////////////////////
  SN_RC_INTF_SHM_ID,	// property to assign shm id from client/manager master thread, liyenho
  PROP_LAST
};
...
static int process_time_toset()
{
   struct timespec start, stop;
   clockid_t clock_id  =  CLOCK_REALTIME;//CLOCK_MONOTONIC;//CLOCK_REALTIME;//
   static long tprevnano = 0,tprevsec = 0;
   double diffnano,diffsec;
   int ret = 0;
   static int t_flag = 1;
   ...
   if (t_flag == 1)
   {
      ...
      tprevnano =  (long)(start.tv_nsec/1000);
      tprevsec = (long)(start.tv_sec);
      t_flag = 0;
      ret = 1;
   }
   else
   {
      ...
      if (stop.tv_nsec >= tprevnano)
         diffnano = difftime(stop.tv_nsec, tprevnano);
      else
         diffnano = difftime(tprevnano, stop.tv_nsec);

      if (stop.tv_sec >= tprevsec)
         diffsec = difftime(stop.tv_sec, tprevsec);
      else
         diffsec = difftime(tprevsec, stop.tv_sec); 

      if ((diffnano < 999076832.00) && (diffsec < 1.00))
         ret = 0;
      else
      {
         clock_gettime(clock_id, &start);
...
/////////////////////////////////// bookmark ////////////////////////////////////////
  g_object_class_install_property (gobject_class,	// register SHMID property, liyenho
      SN_RC_INTF_SHM_ID, g_param_spec_int ("shmid",
          "ShmId-Ctrl-Rc", "ShmId for SN RC_CTRL (1019_10)",
          0, G_MAXINT, UDP_DEFAULT_SHMID, 
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_udpsrc_init (GstUDPSrc * udpsrc, GstUDPSrcClass * g_class)
{
  ...
  udpsrc->shmid = UDP_DEFAULT_SHMID;
  udpsrc->stats_sn = STATS_SN_INVALID; 
...
gst_udpsrc_finalize (GObject * object)
{
  ...
  WSA_CLEANUP (object);
  {
    intf_dec_rc_sn* stats_sn_local;  
    stats_sn_local = (intf_dec_rc_sn*)(udpsrc->stats_sn);

	  if ((STATS_SN_INVALID!=(int)stats_sn_local) && (-1!=(int)stats_sn_local)) {
		  GST_ERROR("detach shm from gstudpsrc"); 
		  shmdt ((void*)stats_sn_local); 
...
gst_udpsrc_create (GstPushSrc * psrc, GstBuffer ** buf)
{
  ...
  int testv;
  struct timespec tm;
  static int tprev,tcurr,tdelta,ttail;
...
retry:
   ...
   * case it can return 0 and we dont want a 0 sized buffer. */
  readsize = 0;
  if (G_UNLIKELY ((ret =
              IOCTL_SOCKET (udpsrc->sock.fd, FIONREAD, &readsize)) < 0))
    goto ioctl_failed;
  ...
  if (G_UNLIKELY (!readsize)) {
    ...
    if (((int)stats_sn_local!=STATS_SN_INVALID) && (-1 !=(int)stats_sn_local)) 
      stats_sn_local->socket_error_cnt++;
    slen = sizeof (sa);
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    default:
      break;
/////////////////////////////////// bookmark ////////////////////////////////////////
    case SN_RC_INTF_SHM_ID:	// fetch shm id from master and attach to it, liyenho
    {
/******************** bookmark ***************snf1*************************/
// NOTE: GST_ELEMENT_ERROR() cannot print data in this section
  intf_dec_rc_sn* stats_sn_local;

  udpsrc->shmid = g_value_get_int (value);
  stats_sn_local = (intf_dec_rc_sn*)(udpsrc->stats_sn);
  //if (stats_sn && -1!=(int)stats_sn_local) {
  //GST_WARNING("GstUdpSrc: Shm isn't null...");
	//shmdt((void*)stats_sn_local); }
  if(stats_sn_local == STATS_SN_INVALID) {
    stats_sn_local = (intf_dec_rc_sn*)shmat(udpsrc->shmid,NULL,0);
    if((unsigned int)stats_sn_local == -1) {
      GST_ELEMENT_ERROR (udpsrc, RESOURCE, SETTINGS, (NULL),
        ("SN_UDP: fail to establish communication link"));}
    #if _USE_SEMAPHORE_
     GST_DEBUG ("GstUdpSrc: Sem ID =%u", stats_sn_local->semid_rec);
	   semid_rec= stats_sn_local->semid_rec;
    #endif
    if (stats_sn_local && -1!=(int)stats_sn_local)  
       stats_sn_local->version = (stats_sn_local->version & 0xff0fffff) | SN_VERSION;
    udpsrc->stats_sn = (void*)stats_sn_local;
  }
/************************************************************************/
    	break;
	  }
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    default:
      ...
      break;
    case SN_RC_INTF_SHM_ID:	// fetch shm id from master and attach to it, liyenho
      g_value_set_int (value, udpsrc->shmid);
      break;
  }
}
...
gst_udpsrc_start (GstBaseSrc * bsrc)
{
  ...
  stats_sn_local = (intf_dec_rc_sn*)(src->stats_sn);
  ...
  len = sizeof (rcvsize);
  if ((STATS_SN_INVALID!=(int)stats_sn_local) && (-1!=(int)stats_sn_local))
    src->buffer_size = stats_sn_local->udpbuffersize;
...
membership:
  {
    CLOSE_IF_REQUESTED (src);
    GST_ELEMENT_ERROR (src, RESOURCE, SETTINGS, (NULL),
        ("couldn't add membership %d: %s (%d)", ret, g_strerror (errno), errno));
    return FALSE;
  }
getsockname_error:
  {
