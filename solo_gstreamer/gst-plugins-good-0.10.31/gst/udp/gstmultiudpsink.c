...
static intf_enc_rc_sn* stats_sn_global = STATS_SN_INVALID;
...
gst_multiudpsink_class_init (GstMultiUDPSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class,	
      SN_RC_INTF_SHM_ID, g_param_spec_int ("shmid",
          "ShmId-Ctrl-Rc", "ShmId for SN RC_CTRL (1211_11)",
...
gst_multiudpsink_init (GstMultiUDPSink * sink)
{
  ...
  sink->shmid = UDP_DEFAULT_SHMID;
  stats_sn_global = STATS_SN_INVALID;
}
...
gst_multiudpsink_finalize (GObject * object)
{
  ...
  {
	  if ((STATS_SN_INVALID!=(int)stats_sn_global) && (-1!=(int)stats_sn_global)) {
		  GST_ERROR("detach shm from gstmultiudpsink"); 
		  shmdt ((void*)stats_sn_global); 
	  }
  }
...
gst_multiudpsink_render (GstBaseSink * bsink, GstBuffer * buffer)
{
  ...
  for (clients = sink->clients; clients; clients = g_list_next (clients)) {
   ...
   // if (size > 4){//test
       ...
       //printf("test for ret = %d  t_count  = %u\n", ret, t_print);
     //} else ret = 0;//end test   
        if (ret < 0) {
           if (((int)stats_sn_global!=STATS_SN_INVALID) && (-1 !=(int)stats_sn_global))
             stats_sn_global->socket_error_cnt++;
...
/**/
  udpsink->shmid = g_value_get_int (value);
  if (stats_sn_global != STATS_SN_INVALID) 
  {
    GST_WARNING("GstUdpSink: Shm isn't null (%d)...",stats_sn_global);
	  shmdt((void*)stats_sn_global); 
  }
  if(stats_sn_global == STATS_SN_INVALID) 
  {
    stats_sn_global = (intf_enc_rc_sn*)shmat(udpsink->shmid,NULL,0);
    if((unsigned int)stats_sn_global == -1) 
    {
      GST_ELEMENT_ERROR (udpsink, RESOURCE, SETTINGS, (NULL),
        ("SN_UDPSINK: fail to establish communication link %d", udpsink->shmid));
    }
                   #if _USE_SEMAPHORE_
                   GST_DEBUG ("GstMultiUdpSink: Sem ID =%u", stats_sn_global->semid_rec);
	                 semid_rec= stats_sn_global->semid_rec;
                   #endif
    if (stats_sn_global && -1!=(int)stats_sn_global)  
       stats_sn_global->version = (stats_sn_global->version & 0xf0ffffff) | SN_VERSION;
  }
/**/
/************************************************************************/
  }
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    default:
      ...
      break;
    case SN_RC_INTF_SHM_ID:	
      g_value_set_int (value, udpsink->shmid);
      break;
  }
}
