 ...
 * signal.
 *
 * Access to the internal statistics of gstrtpbin is provided with the			// this will be very useful insight!@# liyenho
...
GST_STATIC_PAD_TEMPLATE ("recv_rtp_src_%d_%d_%d",
    GST_PAD_SRC,
    GST_PAD_SOMETIMES,	// so this pad will be dynamically created if it's not given... liyenho
    GST_STATIC_CAPS ("application/x-rtp")
    );
...
static intf_dec_rc_sn* stats_sn = 0;	 
#if _USE_SEMAPHORE_
/*static*/ int semid_rtp_rec= -1;
#endif
...
#define DEFAULT_LATENCY_MS           120/*200*/ // would this be a little too much? liyenho, match one in gstrtpjitterbuffer.c
#define DEFAULT_SDES                 NULL
#define DEFAULT_DO_LOST              FALSE
#define DEFAULT_IGNORE_PT            FALSE
#define DEFAULT_NTP_SYNC             FALSE	// we should probably turns this option on, if multiple receivers become active like Yendo said..., liyenho
#define DEFAULT_AUTOREMOVE           FALSE  // why not be TRUE? liyenho
#define DEFAULT_BUFFER_MODE          RTP_JITTER_BUFFER_MODE_SLAVE
#define DEFAULT_USE_PIPELINE_CLOCK   FALSE
#define DEFAULT_RTCP_SYNC            GST_RTP_BIN_RTCP_SYNC_ALWAYS // perfect, I like this! liyenho
...
create_session (GstRtpBin * rtpbin, gint id)
{
  ...
  sess = g_new0 (GstRtpBinSession, 1); // details see line 363, liyenho
...
/////////////////////////////////// bookmark ////////////////////////////////////////
	{ guint32 addr = rtpbin->priv->shmaddr;
	//g_assert(addr && -1!=(int)addr); 
	if (addr && -1!=(int)addr) {
		GST_ERROR("pass shm addr to GstRtpSession"); // for debug, liyenho
   g_object_set (session, "shmaddr", rtpbin->priv->shmaddr, NULL); 
  }}
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    gint64 ts_offset)
{
  ...
  if (prev_ts_offset != ts_offset) {
     ...
     * conversions */
    if (ABS (diff) > 4 * GST_MSECOND) {		// so gstrtpbin tolerate jitter in between 4 msec up to 3 sec (hardwired), huge range then!!! liyenho
    ...
    gint64 rtp_clock_base)
{
  ...
  if (bin->ntp_sync) {
    ...
    for (walk = client->streams; walk; walk = g_slist_next (walk)) {
      ...
      stream_set_ts_offset (bin, ostream, ts_offset);	// all streams (ostream->have_sync == true) shall sync with one with min jitter, liyenho
    }
  }
  return;
}
...
gst_rtp_bin_handle_sync (GstElement * jitterbuffer, GstStructure * s,
    GstRtpBinStream * stream)	// this call was signaled by gstrtpjitterbuff.c @ line 2064, liyenho
{
  GstRtpBin *bin;
...
gst_rtp_bin_base_init (gpointer klass)
{
  ...
  gst_element_class_add_static_pad_template (element_class,
      &rtpbin_recv_rtcp_sink_template); // who taps into this pad? we want to..., liyenho
  gst_element_class_add_static_pad_template (element_class,
...
gst_rtp_bin_class_init (GstRtpBinClass * klass)
{
   ...
   * // we should probably turns this option on, if multiple receivers become active like Yendo said..., liyenho
   * Since: 0.10.21
   */
  ...
  g_object_class_install_property (gobject_class, PROP_RTCP_SYNC_INTERVAL,
	...
	GST_ERROR("install 'shmid' property in gstrtpbin"); 
  g_object_class_install_property (gobject_class,
      SN_RC_INTF_SHM_ID, g_param_spec_int ("shmid",
          "ShmId-Ctrl-Rc", "ShmId for SN RC_CTRL",
...
gst_rtp_bin_init (GstRtpBin * rtpbin, GstRtpBinClass * klass)
{
  ...
  rtpbin->sdes = gst_structure_new ("application/x-rtp-source-sdes",  // not very descriptive?;-) liyenho
...
gst_rtp_bin_finalize (GObject * object)
{
  ...
  g_mutex_free (rtpbin->priv->dyn_lock);
	...
	if (stats_sn && -1!=(int)stats_sn) {
		GST_ERROR("detach shm from gstrtpbin"); 
		shmdt ((void*)stats_sn); 
	}
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case SN_RC_INTF_SHM_ID:	
    {
       gint32 shmid = g_value_get_int (value);
      GST_DEBUG (">>GstRtpBin: Shm ID =%u", shmid);
      if (stats_sn && -1!=(int)stats_sn) {
			GST_WARNING("GstRtpBin: Shm isn't null...");
	      shmdt((void*)stats_sn); }
  		stats_sn = (intf_dec_rc_sn*)shmat(shmid,NULL,0);
  		if((unsigned int)stats_sn == -1) {
    		GST_ERROR ("GstRtpBin: Shared memory attach failed...");
    		g_assert(0);	// don't apply exit() @ run time...
 		}
    	rtpbin->priv->shmaddr = (gint32)stats_sn;
	GST_ERROR("assign shmaddr in gstrtpbin"); 
#if _USE_SEMAPHORE_
    	g_assert (0<stats_sn->semid_rec);
      GST_DEBUG ("GstRtpBin: Sem ID =%u", stats_sn->semid_rec);
	  		semid_rtp_rec= stats_sn->semid_rec;
#endif
    	break;
	 }
  }
    ...
    case GST_MESSAGE_BUFFERING:
    {
      ...
      if (G_LIKELY (stream)) {
        ...
        gst_message_set_buffering_stats (message, mode, avg_in, avg_out,
            buffering_left);  // what is this for, just duplicate what was parsed? liyenho
...
gst_rtp_bin_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_PAUSED_TO_READY:
       ...
       * be able to happen as we set the shutdown flag. */ // CRITICAL CONCEPT...!@#$%^ liyenho
    ...
    GstRtpBinSession * session)
{
  ...
  g_free (padname);
  sinkpad = gst_element_get_static_pad (stream->buffer, "sink"); // get existing sink jitterbuffer obj, liyenho
  ...
  srcpad = gst_element_get_static_pad (element, padname);
  g_free (padname);
  sinkpad = gst_element_get_request_pad (stream->buffer, "sink_rtcp"); // create sink_rtcp jitterbuffer obj, liyenho
  gst_pad_link (srcpad, sinkpad);
  gst_object_unref (sinkpad);
  gst_object_unref (srcpad);
  ...
  if (stream->demux) {
    ...
    pad = gst_element_get_static_pad (stream->buffer, "src");  // get existing src jitterbuffer obj, liyenho
...
create_recv_rtp (GstRtpBin * rtpbin, GstPadTemplate * templ, const gchar * name)
{
  ...
  session->recv_rtp_sink =
      gst_element_get_request_pad (session->session, "recv_rtp_sink"); // create recv_rtp_sink session obj, liyenho
  ...
  session->recv_rtp_src =
      gst_element_get_static_pad (session->session, "recv_rtp_src"); // get existing recv_rtp_src session obj, liyenho
  if (session->recv_rtp_src == NULL)
    goto pad_failed;

  GST_DEBUG_OBJECT (rtpbin, "getting demuxer RTP sink pad");
  sinkdpad = gst_element_get_static_pad (session->demux, "sink");  // get existing sink demux obj, liyenho
    ...
    const gchar * name)
{
  ...
  session->recv_rtcp_sink =
      gst_element_get_request_pad (session->session, "recv_rtcp_sink"); // create recv_rtcp_sink session obj, liyenho
  if (session->recv_rtcp_sink == NULL)
    goto pad_failed;

  /* get srcpad, link to SSRCDemux */
  GST_DEBUG_OBJECT (rtpbin, "getting sync src pad");
  session->sync_src = gst_element_get_static_pad (session->session, "sync_src"); // get existing sync_src session obj, liyenho
  if (session->sync_src == NULL)
    goto pad_failed;

  GST_DEBUG_OBJECT (rtpbin, "getting demuxer RTCP sink pad");
  sinkdpad = gst_element_get_static_pad (session->demux, "rtcp_sink"); // get existing rtcp_sink demux obj, liyenho
...
create_send_rtp (GstRtpBin * rtpbin, GstPadTemplate * templ, const gchar * name)
{
  ...
  session->send_rtp_sink =
      gst_element_get_request_pad (session->session, "send_rtp_sink"); // create send_rtp_sink session obj, liyenho
  ...
  session->send_rtp_src =
      gst_element_get_static_pad (session->session, "send_rtp_src");  // get existing send_rtp_src session obj, liyenho
...
create_rtcp (GstRtpBin * rtpbin, GstPadTemplate * templ, const gchar * name)
{
  ...
  session->send_rtcp_src =
      gst_element_get_request_pad (session->session, "send_rtcp_src"); // create send_rtcp_src session obj, liyenho
    ...
    GstPadTemplate * templ, const gchar * name)
{
  ...
  } // for these types of pads; _recv_rtp_src_%d and _send_rtp_src_%d, see line 163 & 177, liyenho
  else
    goto wrong_template;
