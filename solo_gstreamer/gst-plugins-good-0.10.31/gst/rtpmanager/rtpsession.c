...
#include "stats_sn_gst.h" // added by liyenho
...
rtp_session_init (RTPSession * sess)
{
  ...
  INIT_AVG (sess->stats.avg_rtcp_packet_size, 100); // avg rtcp size to be 100 bytes, liyenho
...
rtp_session_create_source (RTPSession * sess)
{
  ...
  source = rtp_source_new (ssrc);
  rtp_source_set_callbacks (source, &callbacks, sess); // @ line 1259, liyenho
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!sess->source->received_bye && members < pmembers) {
      ...
      if (arrival->current_time < sess->next_rtcp_check_time) {
        ...
        sess->next_rtcp_check_time =		// INDEED THIS IS CRUCIAL TO MAINTAIN RTCP BANDWIDTH WHEN ACTIVE MEMBERS CHANGE! liyenho
    ...
    gboolean fir, GstClockTime current_time)
{
  ...
  RTP_SESSION_UNLOCK (sess);
  sess->callbacks.request_key_unit (sess, fir, // FIR mode requires entire headers resent, liyenho
...
{	// we shall tap into this func to detect IDR request to manager/client, liyenho
  RTPSource *src;
    ...
    guint8 * fci_data, guint fci_length, GstClockTime current_time)
{
  ...
  if (!src && sender_ssrc == 1) {
    ...
    if (sess->stats.sender_sources >
      ...
      return;	// why return if sender_sources is larger than 2/1??? liyenho
    ...
    RTPArrivalStats * arrival, GstClockTime current_time)
{
  ...
  if (g_signal_has_handler_pending (sess,
    ...
    g_signal_emit (sess, rtp_session_signals[SIGNAL_ON_FEEDBACK_RTCP], 0,
        type, fbtype, sender_ssrc, media_ssrc, fci_buffer); // doesn't seem any specific signal handler is hooked?! liyenho
    RTP_SESSION_LOCK (sess);
    ...
    gboolean first)
{
  ...
  if (sess->recalc_bandwidth) {
    ...
    if (sess->bandwidth > 0)
      ...
      g_hash_table_foreach (sess->cnames, (GHFunc) add_bitrates, &bandwidth); // why is src from cnames instead ssrc hash table? liyenho
      bandwidth /= 8.0;
    }
    if (bandwidth < 8000)
      bandwidth = RTP_STATS_BANDWIDTH;

    rtp_stats_set_bandwidths (&sess->stats, bandwidth, // this shall dynamically expand up or shrink down rtp/rtcp bandwidth, liyenho
...
session_start_rtcp (RTPSession * sess, ReportData * data)
{
  ...
  if (RTP_SOURCE_IS_SENDER (own)) {
    ...
    GST_DEBUG ("create SR for SSRC %08x", own->ssrc);
     // regardless what follows this SR, we shall produce a rtcp SR per given time/bandwidth, this's correct, liyenho
    ...
    /* fill in sender report info */ // THIS IS FUNC TO SETUP ALL CRITICAL INFO FOR gstrtpjitterbuffer::SIGNAL_HANDLE_SYNC, liyenho
    gst_rtcp_packet_sr_set_sender_info (packet, own->ssrc,
        ntptime, rtptime, packet_count, octet_count);
  } else {
    /* we are only receiver, create RR */
    GST_DEBUG ("create RR for SSRC %08x", own->ssrc);
     // regardless what rb(s) follow this RR, we shall produce a rtcp RR per given time/bandwidth, this's correct, liyenho
...
session_report_blocks (const gchar * key, RTPSource * source, ReportData * data)
{
  ...
  if (data->rtcp == NULL) {
    session_start_rtcp (sess, data); // client (enc) to be sole sender on session, no sender rep expected..., liyenho
...
/////////////////////////////////// bookmark ////////////////////////////////////////
	{ intf_dec_rc_sn* stats_sn = (intf_dec_rc_sn*) sess->shmaddr;
	  if (stats_sn && -1!=(gint)stats_sn)
		if (!stats_sn->is_sender) {
#if _USE_SEMAPHORE_
		struct sembuf sops[2];
			sops[0].sem_op=0; sops[1].sem_op=1;
			semop(stats_sn->semid_rec,sops,2);
#endif
		GST_ERROR("rtpsession: save various dec info"); // for debug, liyenho
			stats_sn->bitrateall = sess->bandwidth;
   GST_ERROR("rtpsession: stats_sn->bitrateall=%d\n",stats_sn->bitrateall);
			stats_sn->maxseqnum = exthighestseq;
   GST_ERROR("rtpsession: stats_sn->maxseqnum=%d\n",stats_sn->maxseqnum);
			stats_sn->packetlost = packetslost;
   GST_ERROR("rtpsession: stats_sn->packetlost=%d\n",stats_sn->packetlost);
			stats_sn->jitter = jitter;
   GST_ERROR("rtpsession: stats_sn->jitter=%d\n",stats_sn->jitter);
#if _USE_SEMAPHORE_
			sops[0].sem_op=-1;
			semop(stats_sn->semid_rec,sops,1);
#endif
		}}
///////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (!(source == sess->source)) {
    ...
    if (data->current_time > btime) {
      ...
      if (data->current_time - btime > interval) {
        ...
        remove = TRUE;	// this makes sense, cool..., liyenho
      }
    }
  }
...
session_sdes (RTPSession * sess, ReportData * data)
{
  ...
  sdes = rtp_source_get_sdes_struct (sess->source); // default sdes field(s) set by rtpsource.c::rtp_source_init, liyenho
...
early:	// we shall produce a new rtcp packet on this check, liyenho
  ...
  if (sess->stats.min_interval != 0 && !sess->first_rtcp) {
    ...
    if (sess->last_rtcp_send_time + T_rr_current_interval >
      ...
      GST_DEBUG ("RTCP packet could be suppressed min: %" GST_TIME_FORMAT
          ...
          GST_TIME_ARGS (sess->next_rtcp_check_time));
      data->may_suppress = TRUE;	// so the next rtcp packet is subject to be suppressed due to being too frequent, liyenho
    }
  }
...
/**	// THIS IS THE MAIN RTCP PROCESS ENTRY, WE CAN TAP INTO THIS ONE FOR BIT RATE CTRL UPON NETWORK COND !!!@#$%^&*, liyenho
    ...
    guint64 ntpnstime, GstClockTime running_time)
{
  ...
  if (data.rtcp) {
    ...
    if (!data.is_early && !data.may_suppress)
      sess->last_rtcp_send_time = data.current_time; // how about if data.is_early is true? what happen to sess->last_rtcp_send_time? liyenho
    sess->first_rtcp = FALSE;
    sess->next_early_rtcp_time = GST_CLOCK_TIME_NONE;

    /* add SDES for this source when not already added */
    if (!data.has_sdes)
      session_sdes (sess, &data); // THIS IS FUNC TO TRIGGER FOR gstrtpbin::gst_rtp_bin_handle_sync FOR RTCP SYNC, liyenho
  }
